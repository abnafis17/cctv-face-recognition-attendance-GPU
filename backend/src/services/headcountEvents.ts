...
const MAX_EVENTS = Math.max(
  0,
  Number(process.env.HC_EVENTS_MAX ?? 500) || 500,
);
...
export function pushHeadcountEvent(
  companyId: string,
  event: Omit<HeadcountEvent, "seq">,
): number {
...
export async function getHeadcountEvents(params: {
  companyId: string;
  afterSeq?: number;
  limit?: number;
  waitMs?: number;
  signal?: AbortSignal;
}): Promise<{ latest_seq: number; events: HeadcountEvent[] }> {
  const afterSeq = Math.max(0, Number(params.afterSeq ?? 0) || 0);
  const limit = Math.min(Math.max(Number(params.limit ?? 50) || 50, 1), 200);
  const waitMs = Math.min(
    Math.max(Number(params.waitMs ?? 0) || 0, 0),
    300_000,
  );
